...
export const keymap: KeymapSection[] = [
  {
    ...
    bindings: [
      ...
      { keys: ['v', 'g'], action: 'Toggle the grouped list view', note: 'groups from ~/.config/pull-all/groups.json; persisted', keywords: ['group', 'grouping', 'view', 'section'] },
      ...
      { keys: [','], action: 'Open the settings modal', note: 'General: panel padding · grouping · tree view — Theming: icons (Unicode/emoji) · theme (auto/dark/light, auto re-detects live) · background (normal/soft/terminal) · contrast (normal/soft); background and contrast are independent (background = surface tones, contrast = text/accent saturation); rows and radio chips are clickable, [x]/outside-click closes', keywords: ['settings', 'preferences', 'config', 'emoji', 'padding', 'theme', 'background', 'terminal', 'contrast', 'dark mode', 'light mode', 'grouping', 'tree', 'mouse'] },
  ...
  },
  {
    ...
    bindings: [
      ...
      { keys: ['y'], action: 'Copy menu — absolute path / branch name / both' },
